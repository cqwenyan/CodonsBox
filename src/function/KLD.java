...
public class KLD implements Operation {
	...
	final public static String[] Code_table = { "TTT", "TCT", "TTC", "TCC",
			"TTA", "TCA", "TTG", "TCG", "CTT", "CCT", "CTC", "CCC", "CTA",
			"CCA", "CTG", "CCG", "ATT", "ACT", "ATC", "ACC", "ATA", "ACA",
			"ACG", "GTT", "GCT", "GTC", "GCC", "GTA", "GCA", "GTG", "GCG",
			"TAT", "TGT", "TAC", "TGC", "CAT", "CGT", "CAC", "CGC", "CAA",
			"CGA", "CAG", "CGG", "AAT", "AGT", "AAC", "AGC", "AAA", "AGA",
			"AAG", "AGG", "GAT", "GGT", "GAC", "GGC", "GAA", "GGA", "GAG",
			"GGG" };
	...
	public void workNew(String path, String fileName) {
		...
		if (!MainMenu.getPlusfield().equals("0")) {
			...
			KLD_Builder(Integer.valueOf(tempKLD[0]),
					Integer.valueOf(tempKLD[1]), path, fileName);
			cleanBreak.deleteTempFile(path);
		}
	}

	private static void KLD_Builder(int binSize, int binNumber, String path,
		...
		try {
			...
			for (int j = 0; j < Code_table.length; j++) {
				for (int i = 0; i < binNumber; i++) {
					...
					bw.write(new BigDecimal(Double.toString(x)).setScale(3,
